...
The KDE Wallet daemon (kwalletd5 / kwalletd6) must be running and the target
wallet must be unlocked.

An optional companion file ``{stem}_attributes.json`` in the same directory as
the .kwl file can be used to supplement item attributes that kwalletd does not
always expose through its SecretService compatibility layer.  The file is not
required; if absent, only the attributes returned by D-Bus are used.
...
def read_kde_keyring(kwl_path: Path) -> list[dict]:
    ...
    Raises RuntimeError if the kwalletd daemon is not running or the wallet
    collection cannot be found.
    """
    kwl_path = kwl_path.resolve()

    # Build item-label → attributes lookup from the optional JSON index.
    ...
    for collection in collections:
        ...
        for item in collection.get_all_items():
            ...
            # for items whose attributes were not fully exposed via SecretService.
